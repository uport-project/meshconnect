    ...
    if(
      ...
      && this.props.registeredEntityRequest[0]){
        if(this.props.registeredEntityRequest[0]){
          this.props.registeredEntityRequest[0].map(address=>{
...
const mapDispatchToProps = (dispatch, props) => ({
  ...
  departure: (name)=>dispatch(uPortSendTransactionRequest({
    payload: {
      contractABI: PunchTheClock.abi,
      contractAddress: props.contractAddress,
      contractFunction: "depart",
    },
    metadata: {
      delta: 'contract|transaction|depart'
    }
  })),
})
...
const onSubmit = (data, dispatch, props) => new Promise((resolve, reject) => {
  props.entityRegister(data.nameDisplay)
  console.log('wining')
})
